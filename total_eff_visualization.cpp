...
void total_eff_visualization(){
    ...
    TH2D *hist = new TH2D("hist", "Total Efficiency", nbinsx, xlow, xup, nbinsy, ylow, yup);
     std::array<double, 260> total_eff = geom_eff();
        ...
        for (int i = 0; i < 260; ++i) {
         ...
         TCanvas *c = new TCanvas("c", "Total Efficiency Visualization", 1200, 1000);
   ...
   std::vector<TH2D*> hist(42);  // Create space for 42 histograms

for (int i = 0; i < 42; ++i) {
    hist[i] = new TH2D(Form("hist%d", i), "Total Efficiency",
...
TCanvas *c = new TCanvas(Form("c%d", i), "Canvas", 1200, 1000);
c->cd();
hist[i]->Draw("COLZ");
